class Country < ActiveRecord::Base
  extend FriendlyId
  friendly_id :iso3_code, use: %i[slugged finders]
  ...
  def self.count_indicators!(index_name = Rails.application.config.imon['current_index'])
    ...
    all.each do |country|
      ...
      groups = []
      current_indicators.affecting_score.each do |i|
  ...
  def self.calculate_scores_and_rank!(index_name = Rails.application.config.imon['current_index'])
    ...
    with_enough_data.desc_score.each_with_index do |country, i|
      country.rank = i + 1
      country.save!
    end
  end
