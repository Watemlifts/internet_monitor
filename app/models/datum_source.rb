class DatumSource < ActiveRecord::Base
  ...
  scope :are_api, lambda {
    where(is_api: true)
  }

  scope :only_indicators, lambda {
  ...
  def retriever_class(option_class = nil)
    ...
    rc = option_class if option_class.present?
  ...
  def recalc_min_max(index_name)
    ...
    self.min, self.max = temp_data.minmax
  end
  ...
  def ingest_data!(options = {}, datum = nil)
    if datum.present?
      ...
      if options[:append]
        data << retriever_class(options[:retriever_class]).data(options)
      else
      ...
      unless data.empty?
        ...
        logger.info %(Ingested #{data.count} data from "#{options[:retriever_class]}")
        save!
      end
    end
  end
