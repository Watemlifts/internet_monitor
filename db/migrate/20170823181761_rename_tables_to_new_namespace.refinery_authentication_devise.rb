...
class RenameTablesToNewNamespace < ActiveRecord::Migration
  def change
    remove_index :refinery_user_plugins, %i[user_id name]
    add_index :refinery_user_plugins, %i[user_id name], unique: true,
                                                        name: :refinery_user_plugins_user_id_name

    remove_index :refinery_roles_users, %i[role_id user_id]
    remove_index :refinery_roles_users, %i[user_id role_id]
    add_index :refinery_roles_users, %i[role_id user_id], name: :refinery_roles_users_role_id_user_id
    add_index :refinery_roles_users, %i[user_id role_id], name: :refinery_roles_users_user_id_role_id
