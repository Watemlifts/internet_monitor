...
describe('country_categories/show') do
  subject { rendered }

  shared_examples_for('indicators') do
    ...
    context('with some indicators') do
      let(:data) do
  ...
  shared_examples_for('country_category layout') do
    it {
  ...
  shared_examples_for('country sidebar') do
  ...
  context('full country') do
    ...
    context('access') do
      ...
      let(:d_fixed_monthly_country) do
        category.data.indicators.most_recent.for(country).find_by_datum_source_id ds_fixed_monthly.id
      end
      ...
      let(:d_lit_rate_country) do
        category.data.indicators.most_recent.for(country).find_by_datum_source_id ds_lit_rate.id
      end
    ...
    context('activity') do
      ...
      describe('morningside') do
        ...
        it('should have morningside description') {
          ...
          should have_css 'section.morningside-fetcher .body',
                          text: 'Researchers at Berkman are currently working to analyze this data.'
        }

        it('should have a note under the blogosphere') {
          # description comes from cms page side_body
          should have_css 'section.morningside-fetcher .side_body',
                          text: 'For an earlier report on the Arabic blogosphere using similar research methods, see "Mapping the Arabic Blogosphere: Politics, Culture and Dissent" (2009).'
        }
      end

      describe('api widget') do
  ...
  context('country without activity') do
